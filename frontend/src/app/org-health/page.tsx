...
import type { GenerateLeadershipPulseOutput, LeadershipQuestion, SurveyQuestion, DeployedSurvey, SummarizeSurveyResultsOutput } from '../types/ai';
...
function CreateSurveyWizard({ onSurveyDeployed }: { onSurveyDeployed: () => void }) {
  ...
  const handleGenerateQuestions = (currentObjective: string) => {
    ...
    startGeneration(async () => {
      ...
      try {
        const response = await fetch('/api/ai/generate-survey-questions', {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            throw new Error(errorData.error || 'Failed to generate survey questions.');
        }
        
        const result = await response.json();
        const questionsWithIds = result.questions.map((q: SurveyQuestion) => ({ ...q, id: uuidv4() }));
...
function LeadershipPulseDialog({ open, onOpenChange, summary, surveyObjective, onPulseSent }: { open: boolean, onOpenChange: (open: boolean) => void, summary: SummarizeSurveyResultsOutput | null, surveyObjective: string, onPulseSent: () => void }) {
    ...
    const handleGenerateTargetedQuestion = async (roleKey: 'teamLeadQuestions' | 'amQuestions' | 'managerQuestions') => {
        ...
        try {
            const response = await fetch('/api/ai/generate-survey-questions', {
                ...
                body: JSON.stringify({ objective: targetedQuestionInput }),
            });
    ...
    const handleSendToLeaders = async () => {
        ...
        setIsLoading(true);
        try {
            const response = await fetch('/api/ai/generate-leadership-pulse', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.error || 'Failed to generate leadership pulse.');
            }

            // Although the backend returns the generated pulse, we already have it in pulseData
            // The success of the API call is what matters here.
            
            toast({ title: "Leadership Pulse Sent", description: "Leaders have been notified in their Messages." });
            onPulseSent();
            onOpenChange(false);
        } catch (e: any) {
            toast({ variant: 'destructive', title: "Failed to send", description: e.message || "Could not send the leadership pulse." });
        } finally {
            setIsLoading(false);
        }
    };
...
function SurveyResults({ survey, onPulseSent, onSurveyUpdated }: { survey: DeployedSurvey, onPulseSent: () => void, onSurveyUpdated: () => void }) {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            // Fetch raw responses (mocked for now, but would come from a backend DB)
            const rawResponses = mockResponses; // Using the mockResponses defined in this file

            const response = await fetch('/api/ai/summarize-survey-results', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    ...
                    questions: survey.questions.map(q => q.questionText),
                    responses: rawResponses.map(r => Object.values(r)), // Transform mock responses to array of strings
                }),
            });

            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.error || 'Failed to summarize survey results.');
            }

            const result: SummarizeSurveyResultsOutput = await response.json();
            setSummary(result);
            await saveSurveySummary(survey.id, result);
            onSurveyUpdated();

        } catch (e: any) {
            console.error("Failed to analyze survey:", e);
            setError(e.message || "Failed to analyze survey results.");
        } finally {
            setIsLoading(false);
        }
    }
    ...
    const handleAnalyzeLeadershipResponses = async () => {
        ...
        try {
            // Mock data for leadership responses (in a real app, this would be fetched from a DB)
            const leadershipResponses = mockLeadershipResponses; // From the mocked data at the top of the file

            const response = await fetch('/api/ai/summarize-leadership-pulse', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    ...
                    leadershipResponses: leadershipResponses,
                }),
            });

            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.error || 'Failed to summarize leadership pulse.');
            }

            const result = await response.json();
            setCoachingRecs(result.coachingRecommendations);
            await markLeadershipPulseAsAnalyzed(survey.id, result.coachingRecommendations, summary);
            toast({ title: "Analysis Complete", description: "Coaching recommendations have been generated."});
            onSurveyUpdated();

        } catch (e: any) {
            console.error("Failed to analyze leadership responses:", e);
            setError(e.message || "Failed to analyze leadership responses.");
        } finally {
