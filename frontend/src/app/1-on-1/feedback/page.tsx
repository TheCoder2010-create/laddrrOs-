...
import { formSchema } from '../../../backend/src/ai/schemas/one-on-one-schemas'; // Keep formSchema from backend for Zod validation
import type { AnalyzeOneOnOneOutput, CoachingRecommendation } from '../../types/ai'; // Use frontend type
...
function OneOnOneFeedbackForm({ meeting, supervisor }: { meeting: Meeting, supervisor: string }) {
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    ...
    startTransition(async () => {
        try {
            const response = await fetch('/api/ai/analyze-one-on-one', {
                method: 'POST',
                headers: {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.error || 'Failed to analyze 1-on-1 feedback.');
            }
