...
import type { CriticalCoachingInsight, CoachingRecommendation, ActionItem } from '../types/ai';
import { Card, CardContent, CardHeader, CardTitle, CardDescription, CardFooter } from '@/components/ui/card';

import type { BriefingPacketInput, BriefingPacketOutput } from '../types/ai';
...
function BriefingPacketDialog({ meeting, supervisor, viewerRole }: { meeting: Meeting; supervisor: string; viewerRole: Role; }) {
    ...
    const handleOpenChange = (open: boolean) => {
        ...
        try {
            ...
            const employeeName = isEmployeeView ? roleUserMapping[viewerRole]?.name : meeting.with;
            const supervisorName = isEmployeeView ? meeting.with : roleUserMapping[viewerRole]?.name;

            // Fetch necessary data from feedback-service
            const oneOnOneHistory = await getOneOnOneHistory();
            const pastOneOnOneSummaries = oneOnOneHistory
                .filter(item => (item.employeeName === employeeName && item.supervisorName === supervisorName) || (item.employeeName === supervisorName && item.supervisorName === employeeName))
                .map(item => ({
                    supervisorSummary: item.analysis.supervisorSummary,
                    employeeSummary: item.analysis.employeeSummary,
                }));

            const activeCoachingPlans = await getActiveCoachingPlansForUser(isEmployeeView ? viewerRole : role);
            ...
            const outstandingActionItems = aggregatedActionItems
            ...
            const criticalInsights = allFeedback
                .filter(f => f.oneOnOneId && oneOnOneHistory.some(h => h.id === f.oneOnOneId && h.analysis.criticalCoachingInsight?.status !== 'resolved'))
                .map(f => ({
                    summary: f.summary || '',
                    reason: f.criticalityReasoning || '',
                }));

            const input: BriefingPacketInput = {
                supervisorName: supervisorName || '', // Ensure no undefined
                employeeName: employeeName || '', // Ensure no undefined
                ...
                upcomingMeetings: [], // This component doesn't have access to this, so keep it empty for now
            };
            ...
            const result: BriefingPacketOutput = await response.json();
            setPacket(result);
    ...
    }, [supervisor, meeting, viewerRole, role]);
